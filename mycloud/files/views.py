        ...
        if os.path.exists(file_path):
            ...
            if file_type == 'xmind':
                ...
                async with aiofiles.open(file_path, 'w', encoding='utf-8'):
                    pass
        files = await FileExplorer.create2return(id=file_id, name=file_name, format=file_type, parent_id=folder_id, size=0, username=hh.groupname)
...
async def get_all_files(parent_id: str, query: models.SearchItems, hh: models.SessionBase) -> Result:
    ...
    try:
        ...
        if parent_id == 'garbage':
            folders = await FileExplorer.query().equal(username=hh.groupname, status=-1).order_by_key(FileExplorer, order_type).all()
            folder_list = [models.FolderList.from_orm_format(f).model_dump() for f in folders]
        elif parent_id == 'search':
            folders = await FileExplorer.query().equal(username=hh.groupname).not_equal(status=-1).like(name=query.q).order_by_key(FileExplorer, order_type).all()
            folder_list = [models.FolderList.from_orm_format(f).model_dump() for f in folders]
        else:
            folders = await FileExplorer.query().equal(parent_id=parent_id, username=hh.groupname).not_equal(status=-1).order_by_key(FileExplorer, order_type).all()
            folder_list = [models.FolderList.from_orm_format(f).model_dump() for f in folders]
...
async def rename_file(query: models.FilesBase, hh: models.SessionBase) -> Result:
    ...
    try:
        ...
        file = await FileExplorer.get_one(query.id)
...
async def get_file_path(file_id: str, hh: models.SessionBase) -> Result:
    ...
    try:
        file = await FileExplorer.get_one(file_id)
        folder = await FileExplorer.get_one(file.parent_id)
...
async def download_file(file_id: str, hh: models.SessionBase) -> dict:
    file = await FileExplorer.get_one(file_id)
    result = {'path': await file.full_path(), 'name': file.name, 'format': file.format}
...
async def zip_file(query: models.DownloadFile, hh: models.SessionBase) -> Result:
    ...
    try:
        if query.file_type == 'folder':
            files = await FileExplorer.query().equal(parent_id=query.ids[0]).all()
            folder = await FileExplorer.get_one(query.ids[0])
...
async def share_file(query: models.ShareFile, hh: models.SessionBase) -> Result:
    ...
    try:
        file = await FileExplorer.get_one(query.id)
        share = await Shares.create2return(file_id=file.id, name=file.name, path=await file.full_path(),
...
async def move_to_folder(query: models.CatalogMoveTo, hh: models.SessionBase) -> Result:
    ...
    try:
        ...
        if len(query.to_id) <= 3:
            ...
            result.msg = Msg.AccessPermissionNon.get_text(hh.lang)
            return result
        to_folder = await FileExplorer.get_one(query.to_id)
        to_path = await to_folder.full_path()
...
async def upload_file_by_path(query: models.ImportLocalFileByPath, hh: models.SessionBase) -> Result:
    try:
        to_folder = await FileExplorer.get_one(query.id)
        to_path = await to_folder.full_path()
...
async def markdown_to_html(file_id: str, hh: models.SessionBase) -> dict:
    ...
    try:
        ...
        file_path = await file.full_path()
        async with aiofiles.open(file_path, 'r', encoding='utf-8') as f:
            result['data'] = md_to_html(await f.read())
...
async def get_shortcuts(hh: models.SessionBase) -> Result:
    result = Result()
    try:
        files = await Shortcuts.query().equal(username=hh.username).order_by(Shortcuts.id.asc()).all()
...
async def set_shortcuts(file_id: str, hh: models.SessionBase) -> Result:
    ...
    try:
        file = await FileExplorer.get_one(file_id)
        await Shortcuts.create(file_id=file.id, name=file.name, format=file.format, username=hh.username)
...
async def delete_shortcuts(file_id: int, hh: models.SessionBase) -> Result:
    result = Result()
    try:
        await Shortcuts.query().equal(id=file_id).delete()
...
async def save_shared_to_myself(share_id: int, folder_id: str, hh: models.SessionBase) -> Result:
    result = Result()
    try:
        share = await Shares.get_one(share_id)
        file = await FileExplorer.get_one(share.file_id)
