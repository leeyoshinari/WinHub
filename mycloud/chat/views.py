...
async def create_code(chat_mode: int, hh: models.SessionBase) -> Result:
    ...
    try:
        ...
        await ChatRoom.create(code=room_code, mode=chat_mode)
...
async def start_chat(room_code: str, chat_mode: int, hh: models.SessionBase) -> Result:
    ...
    try:
        if room_code not in rooms:
            room = await ChatRoom.query().equal(code=room_code, mode=chat_mode, end_time=0).all()
            ...
            await ChatRoom.update(room[0].id, start_time=int(time.time()))
...
async def delete_code(room_id: int, hh: models.SessionBase) -> Result:
    result = Result()
    try:
        room = await ChatRoom.get_one(room_id)
        await ChatRoom.query().equal(id=room.id).delete()
...
async def clear_expire_chat_code():
    try:
        room = await ChatRoom.query().equal(start_time=0).all()
        for r in room:
            if time.time() - r.create_time.timestamp() > 172800:
                await ChatRoom.query().equal(id=r.id).delete()
...
async def leave_room(room_code, websocket: WebSocket, username: str):
    if room_code in rooms:
        ...
        if not rooms[room_code]["users"]:
            room = await ChatRoom.query().equal(code=room_code, end_time=0).first()
            del rooms[room_code]
            await ChatRoom.update(room.id, end_time=int(time.time()))
