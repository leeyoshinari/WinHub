...
async def get_rank(game_type: str, hh: models.SessionBase) -> Result:
    result = Result()
    try:
        game = await Games.query().equal(type=game_type).order_by(Games.score.desc()).limit(5).all()
...
async def set_score(query: models.GamesScoreInfo, hh: models.SessionBase) -> Result:
    result = Result()
    try:
        game = await Games.query().equal(type=query.type, username=hh.username).order_by(Games.score.desc()).all()
        if game:
            if game[0].score < query.score:
                await Games.update(game[0].id, score=query.score)
            else:
                result.msg = Msg.GameScore.get_text(hh.lang).format(Msg.Success.get_text(hh.lang))
        else:
            await Games.create(type=query.type, username=hh.username, score=query.score)
        result.msg = Msg.GameScore.get_text(hh.lang).format(Msg.Success.get_text(hh.lang))
